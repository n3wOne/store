...
import {
  CART_ITEM, ROUTE_TO_CHECKOUT, CURRENCY, CART_TOTAL, ORDER_BUTTON,
} from '../Constants';
...
const CartContainer = (props) => {
  const prepareChildren = () => {
    const {
      cart, addToCart, removeFromCart, decreaseItem,
    } = props;
  ...
  return <div className="cart-container">{prepareChildren()}
      <div className="cart-total">{CART_TOTAL}: {props.cartTotal} {CURRENCY}</div>
      <div className="cart-order-button"><Link to={ROUTE_TO_CHECKOUT}><button>{ORDER_BUTTON}</button></Link></div>
  </div>;
};
